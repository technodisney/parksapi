...
export class OceanPark extends Destination {
  ...
  async _fetchEntityList(sortId) {
    ...
    return resp?.body?.data?.entityList || [];
  }
  ...
  async buildAttractionEntities() {
    ...
    for (const entity of [...rides, ...transport]) {
      ...
      // FastPass (Ocean FasTrack)
      if (Array.isArray(entity.relateList) && entity.relateList.some((r) => r.type === 'ticket')) {
  ...
  async buildShowEntities() {
    ...
    return shows.map((entity) => {
      ...
      if (coords) built.location = coords;
      return built;
    });
  }
